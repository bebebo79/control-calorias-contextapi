...
export default function ActivityList() {
  ...
  // llamamos state y dispacht
  const {state, dispatch, isEmptyActivities, nameCategory } =useActivity()
  ...
  return (
    <>
        ...
        {isEmptyActivities ? <p className="text-center py-5 text-2xl">No tienes registros aun...</p> : 
            ...
            <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between shadow">
              <div className="space-y-2 relative">
                <p className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold ${activity.category ===1 ? 'bg-lime-500' : 'bg-orange-500'}`}>
                  {nameCategory(+activity.category)}
                </p>
